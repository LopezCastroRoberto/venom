...
for shape in $shapes; do
    ...
    ./build/src/benchmark_spmm --sparsity-type csr --spmm cuSparseLt --gemm cuBlas --precision half --m $m --k $k --n $n --d 0.5 --check >> $log_file
done;
...
cd ./include/spatha/block_sparse/spmm/
mv blockwise_kernel.h blockwise_kernel_128b.h
mv blockwise_kernel_ideal.h blockwise_kernel.h
cd ../../../../build
make clean
...
for mm_row in 10 20 40 100; do
    for shape in $shapes; do
        ...
        for cfg in $cfgs_min1; do
            ...
            ./build/src/benchmark_spmm --sparsity-type n-to-m --spmm spatha --gemm cuBlas --precision half --meta-block-size 32 --block-size 4 --nn_row 2 --mm_row $mm_row --m $m --k $k --n $n --d 0.5 --bm $bm --bn $bn --bk $bk --wm $wm --wn $wn --wk $wk --mm $mm --mn $mn --mk $mk --nstage $nstage >> $log_file
        done;
        IFS=" ";
    done;
    IFS=" ";
...
cd ./include/spatha/block_sparse/spmm/
mv blockwise_kernel.h blockwise_kernel_ideal.h
mv blockwise_kernel_128b.h blockwise_kernel.h
cd ../../../../build
make clean
cmake .. -DCMAKE_BUILD_TYPE=Debug -DCUDA_ARCHS="86" && make -j 16
cd ..
...
for mm_row in 10 20 40 100; do
    for shape in $shapes; do
        ...
        for cfg in $cfgs_min1; do
            ...
            ./build/src/benchmark_spmm --sparsity-type n-to-m --spmm spatha --gemm cuBlas --precision half --meta-block-size 32 --block-size 4 --nn_row 2 --mm_row $mm_row --m $m --k $k --n $n --d 0.5 --bm $bm --bn $bn --bk $bk --wm $wm --wn $wn --wk $wk --mm $mm --mn $mn --mk $mk --nstage $nstage --check >> $log_file
        done;
        IFS=" ";
    done;
    IFS=" ";
