...
mkdir -p result
log_file=result/spmm_bert_spatha.csv
...
mv blockwise_kernel_baseline_32b.h blockwise_kernel.h
cd ../common
...
cd include/spatha/block_sparse/spmm/
mv blockwise_kernel.h blockwise_kernel_baseline_32b.h
mv blockwise_kernel_baseline.h blockwise_kernel.h
cd ../common
...
cd include/spatha/block_sparse/spmm/
mv blockwise_kernel.h blockwise_kernel_baseline.h
...
for shape in $shapes; do
    ...
    for d in 0.5 0.29 0.25 0.2 0.1 0.05 0.02; do
        for v in 2 4 8; do
            echo $d $v
            ./build/src/benchmark_spmm --sparsity-type cvs --spmm CLASP --gemm cuBlas --precision half --block-size $v --m $m --k $k --n $n --d $d >> $log_file
        done;
    done;
    IFS=" ";
done;
...
for shape in $shapes; do
    ...
    IFS=" ";
    for d in 0.5 0.29 0.25 0.2 0.1 0.05 0.02; do
        ./build/src/benchmark_spmm --sparsity-type csr --spmm sputnik --gemm cuBlas --precision half --m $m --k $k --n $n --d $d >> $log_file
    done;
    IFS=" ";
done;
