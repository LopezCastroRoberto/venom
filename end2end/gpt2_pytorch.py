...
class NMVectorSparsifier:
    ...
    def __call__(self, tensor, grad_fmt=None):
        ...
        # uncomment to use random pruning -> mask, columns
...
""" def report_time(name, data, number):
    ...
    if name == "n:m":
        print(
            f"Configuration summary ({name}): n {n} m {m} g {v}."
        )
    print(
        f"Performance summary ({name}): Mean: {mean} ms. Median: {median} ms. Std: {std} ms Repeats {len(ds)}."
    ) """
...
def report_time(name, data, number):
    ...
    if name == "n:m":
        ...
        cfg = "0,0,"
    print(
        "1,"+cfg+str(v)+","+str(mean)+","+str(median)+","+str(std)+","+str(len(ds))
    )
...
def transformer_encoder_layer_prototype(num_repeats, number):
    ...
    model = torch.hub.load('huggingface/pytorch-transformers', 'model', 'gpt2-large')
    model2 = torch.hub.load('huggingface/pytorch-transformers', 'model', 'gpt2-large').to(device='cuda:0').half()
    input = torch.randint(low=0, high=100, size=(8, 512))#, dtype=torch.half)
