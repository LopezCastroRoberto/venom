...
INTERPOLATION_FUNCS = ["linear", "cubic", "inverse_cubic", "nm_decay"]
...
def interpolate(
    ...
    try:
        ...
        x_per=0
    #nm_structure_decay = [0, 1-9/16, 1-7/16, 1-5/16, 1-4/16, 1-3/16, 1-2/16]
    #nm_structure_decay = [0, 1-3/8, 1-2/8]

    #nm_structure_decay = [0, 0.125, 0.125]
    #nm_structure_decay = [0, 1]
    #nm_structure_decay = [0, 0, 1, 1]

    nm_structure_decay = [0, 0.125, 0.25, 0.375]
    #nm_structure_decay = [0]
    ...
    if inter_func == "linear":
        ...
        y_per = 1 - (1 - x_per) ** (1 / 3)
    elif inter_func == "nm_decay":
        ###_LOGGER.info(str(x_cur)+" "+str(x0))
        y_per = nm_structure_decay[int(x_cur - x0)]
        #y_per = nm_structure_decay[int(x_cur - x0)//2]
        #y_per = nm_structure_decay[int(x_cur - x0)//4]
        #_LOGGER.info(str(x_cur - x0) + " " + str(y_per))
