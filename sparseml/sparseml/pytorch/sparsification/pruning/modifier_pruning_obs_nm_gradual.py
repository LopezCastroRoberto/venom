...
@PyTorchModifierYAML()
class OBSnmgPruningModifier(BaseGradualPruningModifier):
    """
    ...
    def initialize(
        ...
        **kwargs,
        ...
        """
        _LOGGER.info("Initializing OBSnmgPruningModifier")
        if (
    ...
    def _get_scorer(self, params: List[Parameter]) -> PruningParamsGradScorer:
        ...
        """
        return OBSnmgPruningParamsScorer(
            params=params,
...
class OBSnmgPruningParamsScorer(PruningParamsGradScorer):
    """
    ...
    def score_parameters(self) -> List[Tensor]:
        ...
        #nm = {0.0:(11,16), 0.6875:(12,16), 0.75:(13,16)}
        ...
        if self._is_main_proc:
            ...
            e_nm = torch.ones((vector_comb, m), dtype=torch.long, device=self._devices[0])
            ...
            for i, finv in enumerate(self._finvs):
                ...
                best_to_prune_idx = torch.zeros(block_w.shape[0], device=self._devices[i])
                best_block_finv_w = torch.zeros((block_w.shape[0], n), device=self._devices[i])
                for k in range(num_combs):
                    #block_w_nm = torch.masked_select(block_w, e_nm[k].unsqueeze_(0).bool()).view(-1, n)
                    block_w_nm = block_w[:, e_nm[k].nonzero().squeeze()] # faster
                    #block_w_nm3 = block_w[:, e_nm[k].ne(0)] # less memory
                ...
                block_finv_w[i] = torch.stack([e_nm[idx] for idx in best_to_prune_idx.to(dtype=torch.long)]).float()
                block_finv_w[i][block_finv_w[i] == 1] = best_block_finv_w.flatten()
