...
@PyTorchModifierYAML()
class OBSnmpairPruningModifier(BaseGradualPruningModifier):
    """
    ...
    def initialize(
        ...
        **kwargs,
        ...
        """
        _LOGGER.info("Initializing OBSnmpairPruningModifier")
        if (
    ...
    def _get_scorer(self, params: List[Parameter]) -> PruningParamsGradScorer:
        ...
        """
        return OBSnmpairPruningParamsScorer(
            params=params,
...
class OBSnmpairPruningParamsScorer(PruningParamsGradScorer):
    """
    ...
    @torch.no_grad()
    def find_best_combination(self, scores, mm_row, nn_row, tensor_shape, device_, dtype_):
        ...
        set_a, indices_a = torch.sort(scores[:, :mm_row//2], 1)
        set_b, indices_b = torch.sort(scores[:, mm_row//2:], 1)
    ...
    def score_parameters(self) -> List[Tensor]:
        ...
        # Change depending on your n:m patter. Examples:
        ...
        if self._is_main_proc:
            for i, finv in enumerate(self._finvs):
                ...
                best_to_prune_idx = torch.zeros(block_w.shape[0], dtype=torch.long)
                ...
                for k in range(num_combs):
                    ...
                    lt = torch.lt(score_nm,best_score_nm)
                    best_score_nm = torch.where(lt, score_nm, best_score_nm)
                    best_to_prune_idx[lt] = k
                    best_block_finv_w[lt,:] = block_finv_w_nm[lt, :]
                    if i==3 or i==0:
                ...
                sets = torch.cat((best_pair_scores, best_indv_scores), dim=1)

                block_finv_w_pair = block_finv_w_pair.reshape(-1,mm_row//2, 2).reshape(-1,mm_row)
                block_finv_w_nflatten = best_block_finv_w.view(-1,mm_row//2)

                tensor_shape = (self._params[i].shape[0]*self._params[i].shape[1]//mm_row, mm_row)

                #block_finv_w[i] = self.find_best_combination(block_finv_w_pair, block_finv_w_nflatten, sets, mm_row, nn_row, tensor_shape, best_to_prune_idx.view(-1,mm_row//2), "cpu").to(self._devices[i])
                best_indices_a2, best_indices_b2 = self.find_best_combination(sets, mm_row, nn_row, tensor_shape, "cpu", block_finv_w_pair.dtype)

                block_finv_w0 = torch.zeros(tensor_shape, dtype=block_finv_w_pair.dtype)
                block_finv_w0[best_indices_a2] = block_finv_w_pair[best_indices_a2]

                best_to_prune_nm_idx = best_to_prune_idx.view(-1, mm_row//2)
                val = block_finv_w_nflatten[best_indices_b2]
                idx = best_indices_b2.nonzero()
                plus = best_to_prune_nm_idx[idx[:,0], idx[:,1]]
                block_finv_w0[idx[:,0],idx[:,1]*2+plus] = val
                ...
                """ if i==0:
                    _LOGGER.info(str(block_w.view(-1, mm_row)[:4]))
                    _LOGGER.info(str(block_finv_w_pair[:4])) """
                """ _LOGGER.info(str(sets[:4]))
                _LOGGER.info(str(block_finv_w_nflatten[:4]))
                _LOGGER.info(str(self._masks[i].reshape(-1, mm_row//2)[:4]))
                _LOGGER.info(str(best_to_prune_idx.view(-1, mm_row//2)[:4]))
                _LOGGER.info(str(block_finv_w[i][:4])) """

                #best_pruned = block_finv_w[i].numpy()
                """ if i==0:
                    for idx, subm in enumerate(block_finv_w[i].reshape(-1, 1, mm_row)):
                        total = torch.sum((~subm.any(axis=0)))
                        if total!=(mm_row-nn_row):
                            _LOGGER.info("error "+str(total)+" in "+str(idx))
                            break """
                ...
                """ for idx, subm in enumerate(scores[i].reshape(-1, 1, mm_row)):
                        total = torch.sum((~subm.any(axis=0)))
                        if(total!=(mm_row-nn_row)):
                            _LOGGER.info("assert " + str(total)+" in "+str(idx))
                        assert( total==(mm_row-nn_row) ) """

                subm = scores[i].reshape(-1, 1, mm_row)
                ok = (torch.sum((~subm.any(axis=1)), dim=1) != (mm_row-nn_row)).any()
                assert (not ok)
    ...
    def mask_update(self, masks: List[Tensor], mask_diffs: List[Tensor]):
        ...
        if self._is_main_proc:
            for i, param in enumerate(self._params):
                ...
                if i==0 or i==3:
                    _LOGGER.info("mask df "+str(mask_diffs[i].reshape(-1,8)[:4]))
