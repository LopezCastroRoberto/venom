...
@PyTorchModifierYAML()
class OBSnmvPruningModifier(BaseGradualPruningModifier):
    """
    ...
    def initialize(
        ...
        **kwargs,
        ...
        """
        _LOGGER.info("Initializing OBSnmvPruningModifier")
        if (
    ...
    def _get_scorer(self, params: List[Parameter]) -> PruningParamsGradScorer:
        ...
        """
        return OBSnmvPruningParamsScorer(
            params=params,
...
class OBSnmvPruningParamsScorer(PruningParamsGradScorer):
    """
    ...
    def score_parameters(self) -> List[Tensor]:
        ...
        nm = {0.0:(12,16), 0.75:(13,16), 0.8125:(14,16)}
        nm = {0.0: (2,8), 0.25: (3,8), 0.375:(4,8), 0.5:(5,8), 0.625:(6,8)}

        sparsity = self._last_applied_sparsity
        n,m = nm[sparsity]
        _LOGGER.info("_last_applied_sparsity "+ str(n) + " " + str(m) + " " + str(sparsity))
        v = 64
        m_p = (m-n+2) if ((m-n)>3) else 4

        vector_comb = comb(m_p,m-n)
        cols_comb = comb(m, m_p)


        if self._is_main_proc:
            """ with torch.profiler.profile(
            schedule=torch.profiler.schedule(wait=0, warmup=0, active=1, repeat=2),
            on_trace_ready=torch.profiler.tensorboard_trace_handler('/users/rlopezca/Documents/Git/sparseml/log/bert-base_v/'),
            record_shapes=True,
            profile_memory=True,
            with_stack=True
            ) as prof: """
            for i, finv in enumerate(self._finvs):
                block_w = self._params[i].data.view(-1, m).to(self._devices[i])
                block_finv = (
                    torch.cat(
                        [
                ...
                nrows, ncols = self._params[i].shape

                best_comb = torch.ones((nrows//v, ncols//m), device=self._devices[i])*torch.inf
                block_finv_w[i] = torch.ones((block_w.shape[0], m), device=self._devices[i])

                for opts in set(list(itertools.combinations(list(range(m)), m_p))):
                    e_nm = torch.ones((vector_comb, m), dtype=torch.long, device=self._devices[i])

                    vec_comb=0
                    for n_opts in set(list(itertools.combinations(list(range(m_p)), m-n))):
                        for ii in list(n_opts):
                            e_nm[vec_comb,opts[ii]] = 0
                        vec_comb+=1
                    num_combs = vector_comb

                    ee_nm = torch.einsum("bij,bkj->bik", e_nm.view(num_combs,m,1), e_nm.view(num_combs, m, 1))

                    best_score_nm = torch.ones(block_w.shape[0], device=self._devices[i])*torch.inf
                    best_block_finv_w = torch.zeros((block_w.shape[0], m), device=self._devices[i])
                    for k in range(num_combs):
                        ...
                        if lt.any():
                            ...
                            best_block_finv_w[lt, :] = e_nm[k].float()
                            best_block_finv_w[best_block_finv_w == 1] = block_finv_w_nm[lt, :].flatten()

                    val = torch.stack([torch.sum(s,0) for s in torch.split(best_score_nm.view(-1, ncols//m), v)])

                    lt = torch.lt(val,best_comb)
                    if lt.any():
                        best_comb = torch.where(lt, val, best_comb)

                        w = lt.unsqueeze(1).repeat(1,v,1).flatten()

                        block_finv_w[i][w, :] = best_block_finv_w[w, :]
    ...
    def mask_update(self, masks: List[Tensor], mask_diffs: List[Tensor]):
        ...
        if self._is_main_proc:
            for i, param in enumerate(self._params):
                _LOGGER.info("assert in " + str(i) + str(torch.all((mask_diffs[i] == -1) == (self._block_finv_w[i].view(mask_diffs[i].shape) != 0))))
