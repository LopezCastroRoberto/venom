...
export RECIPE=research/optimal_BERT_surgeon_oBERT/recipes/30epochs_8block75_squad.yaml
...
python3.10 -m torch.distributed.launch --nproc_per_node=3 src/sparseml/transformers/question_answering.py \
  ...
  --num_train_epochs 30 \
  --recipe ${RECIPE} \
  --output_dir integrations/huggingface-transformers/output_dir/block8 \
