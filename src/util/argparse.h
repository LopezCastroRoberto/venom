...
void parseArgs(const int argc, const char** argv, int &m, int &n, int &k, float &density, int &block_sz, int &meta_block_sz, int &spmm_code, int &gemm_code, int &pattern_code, int &precision_code, unsigned &seed, bool &check, int &bm, int &bn, int &bk, int &wm, int &wn, int &wk, int &mm, int &mn, int &mk, int &nstage, int &nn_row, int &mm_row, bool &load_pattern, std::string &path_to_dset, bool verbose=false)
{
    ...
    block_sz = 1; spmm_code=-1; gemm_code=-1; pattern_code = -1; precision_code=-1; meta_block_sz=0;
    check=false;
    bm=0, bn=0, bk=0;
    wm=0, wn=0; wk=0;
    mm=0, mn=0, mk=0;
