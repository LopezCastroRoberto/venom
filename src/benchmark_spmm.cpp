...
template<typename T, typename T2>
Gemm<T,T2>* create_gemm(int gemm_code, std::vector<T>& A_dense, Dataset<T,T2> &d, cudaDataType_t S, cublasComputeType_t C_gemm){
    Gemm<T,T2>* gemm;
    ...
    {
    case 0:
        gemm = new Cublas_gemm<T,T2>(A_dense, d);
        break;
    case 1:
        gemm = new CublasLt_gemm<T,T2>(A_dense, d, 1, S, C_gemm);
        break;
...
void launch_kernels(int pattern_code, int gemm_code, int spmm_code, int check, int m, int n, int k, float density, int block_sz, int meta_block_sz, int bm, int bn, int bk, int wm, int wn, int wk, int mm, int mn, int mk, int nstage, int nn_row, int mm_row, int seed, cudaDataType_t S, cublasComputeType_t C_gemm, cusparseComputeType C_spmm){
    ...
    Spmm_CNN<T,T2> *spmm = create_spmm(spmm_code, *dt, S, C_spmm);

    Gemm<T,T2> *gemm;
    if(spmm_code==3){
        gemm = create_gemm(gemm_code, spmm->get_sparse(), *dt, S, C_gemm);
    } else {
        gemm = create_gemm(gemm_code, fm->to_dense(), *dt, S, C_gemm);
    }
...
int main(int argc, const char **argv) {
    ...
    parseArgs(argc, argv, m, n, k, density, block_sz, meta_block_sz, spmm_code, gemm_code, pattern_code, precision_code, seed, check, bm, bn, bk, wm, wn, wk, mm, mn, mk, nstage, nn_row, mm_row, load_pattern, path_to_dset, /* verbose */false);

    switch (precision_code)
    {
        case 0:
            launch_kernels<float, float>(pattern_code, gemm_code, spmm_code, check, m, n, k, density, block_sz, meta_block_sz, bm, bn, bk, wm, wn, wk, mm, mn, mk, nstage, nn_row, mm_row, seed, CUDA_R_32F, CUBLAS_COMPUTE_32F, CUSPARSE_COMPUTE_TF32_FAST);
            break;
        case 1:
            launch_kernels<half, half>(pattern_code, gemm_code, spmm_code, check, m, n, k, density, block_sz, meta_block_sz, bm, bn, bk, wm, wn, wk, mm, mn, mk, nstage, nn_row, mm_row, seed, CUDA_R_16F, CUBLAS_COMPUTE_16F, CUSPARSE_COMPUTE_16F);
            break;
        case 2:
            launch_kernels<int8_t, int8_t>(pattern_code, gemm_code, spmm_code, check, m, n, k, density, block_sz, meta_block_sz, bm, bn, bk, wm, wn, wk, mm, mn, mk, nstage, nn_row, mm_row, seed, CUDA_R_8I, CUBLAS_COMPUTE_32I, CUSPARSE_COMPUTE_32I);
            break;
    }
