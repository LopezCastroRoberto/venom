...
CublasLt_gemm<T,T2>::CublasLt_gemm(std::vector<T>& A_dense, Dataset<T,T2> &d, int batch_size, cudaDataType_t Tp, cublasComputeType_t S)
    :Gemm<T,T2>(A_dense, d)
{
    /* A = A_dense;
    B = d.get_B();
...
template<class T, class T2>
CublasLt_gemm<T,T2>::~CublasLt_gemm(){
...
template<class T, class T2>
float CublasLt_gemm<T,T2>::sgemm(int times, int num_batches){
    float time=0;
...
template class CublasLt_gemm<int8_t, int8_t>;
template class CublasLt_gemm<__half, __half>;
template class CublasLt_gemm<float, float>;
