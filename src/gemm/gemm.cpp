...
Gemm<T,T2>::Gemm(std::vector<T>& A_dense, Dataset<T,T2> &d)
    :dset(d)
{
    A = A_dense;
    B = d.get_B();
...
Gemm<T,T2>::Gemm(std::vector<T>& A_dense, Dataset<T,T2> &d, int batch_size, cudaDataType_t Tp, cublasComputeType_t S)
    :dset(d)
{
    A = A_dense;
    B = d.get_B();
...
template<class T, class T2>
std::vector<T2>& Gemm<T,T2>::get_C(){
    return C;
}

template class Gemm<int8_t, int8_t>;
