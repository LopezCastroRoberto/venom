...
void Format_sr_nm<T>::init(int nrow_, int ncol_, int n_, float density_, unsigned seed_, bool row_permute_, int m_, int brow_, int mbrow_, int bm_){
    ...
    /* for(auto i=0; i<this->A_num_rows; i++){
        for(auto j=0; j<A_num_cols_sp_pad; j++){
            if(j<this->A_num_cols_sp){
                this->hA_values[i*A_num_cols_sp_pad+j] = static_cast<T>((float)(std::rand() % 8 + 1));
            } else {
                this->hA_values[i*A_num_cols_sp_pad+j] = static_cast<T>(1.0f);
            }
        }
    } */
...
PYBIND11_MODULE(format, m) {
    ...
    using T = float;
    //using T = half;
    //py::bind_vector<std::vector<T>>(m, "VectorFloat", py::buffer_protocol());

    py::class_<Format<T>>(m, "Format_Float");

    py::class_<Format_sr_nm<T>, Format<T>>(m, "Format_sr_nm_Float")
        .def(py::init<int, int, int, int, int, float, unsigned>())
        .def("init", &Format_sr_nm<T>::init)
        .def("to_dense", &Format_sr_nm<T>::to_dense)
        .def_property_readonly("nrows", &Format<T>::get_A_num_rows)
        .def_property_readonly("ncols", &Format<T>::get_A_num_cols)
        .def_property_readonly("metadata", &Format_sr_nm<T>::get_hA_metadata)
        .def_property_readonly("indices", &Format_sr_nm<T>::get_hA_indices)
        .def_property_readonly("values", &Format_sr_nm<T>::get_hA_val)
        .def_property_readonly("n", &Format_sr_nm<T>::get_n)
        .def_property_readonly("m", &Format_sr_nm<T>::get_m)
        .def_property_readonly("bm", &Format_sr_nm<T>::get_bm);
}
*/